...
(function () {
	...
	function getWebSocket(url, protocols) {
		let parsedUrl = getServerUrl(url);
	...
	class Wampy {
		...
		_wsOnClose(event) {
			...
			if (this._isInitialized && this._options.autoReconnect && this._cache.reconnectingAttempts < this._options.maxRetries) {
				this._cache.timer = window.setTimeout(() => {
		...
		call(procURI, callbacks) {
			let callId = generateId(), i,
				l = arguments.length,
		...
		subscribe(topicURI, callback) {
			let uri = this._prefixMap.resolve(topicURI);
		...
		unsubscribe(topicURI, callback) {
			let i, uri = this._prefixMap.resolve(topicURI);
		...
		publish(topicURI, event, exclude, eligible) {
			let msg = [WAMP_SPEC.TYPE_ID_PUBLISH, topicURI, event];
